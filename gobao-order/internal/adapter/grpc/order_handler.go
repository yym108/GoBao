...
type orderUseCase interface {
	// CreateOrder 创建单独立商品版本订单，并返回完整订单聚合。
	...
	// AdminListOrders 管理员分页查询全部订单（可选状态与用户过滤）。
	AdminListOrders(ctx context.Context, status string, userID int64, page, pageSize int) ([]*domain.Order, int64, error)
	// AdminGetOrderByID 管理员查询任意订单详情。
	AdminGetOrderByID(ctx context.Context, orderID int64) (*domain.Order, error)
	// AdminCancelOrder 管理员关闭任意未支付订单。
	AdminCancelOrder(ctx context.Context, orderID int64) (*domain.Order, error)
}
...
// 当前阶段只开放单独立商品下单，请求中的价格与规格不接受前端传入，全部以后端商品真值为准。
func (h *OrderHandler) CreateOrder(ctx context.Context, req *orderv1.CreateOrderRequest) (*orderv1.CreateOrderResponse, error) {
	...
	if req.GetProductId() <= 0 {
		return nil, pkgerrors.ToGRPCStatus(pkgerrors.New(pkgerrors.CodeInvalidArg, "product_id 必须为正数")).Err()
	}
...
// AdminListOrders 管理员分页查询全部订单 RPC（可选状态过滤，不限定用户）。
func (h *OrderHandler) AdminListOrders(ctx context.Context, req *orderv1.AdminListOrdersRequest) (*orderv1.ListOrdersResponse, error) {
	items, total, err := h.orderUC.AdminListOrders(ctx, req.GetStatus(), req.GetUserId(), int(req.GetPage()), int(req.GetPageSize()))
	if err != nil {
		return nil, pkgerrors.ToGRPCStatus(err).Err()
	}
	pbItems := make([]*orderv1.Order, 0, len(items))
	...
	return &orderv1.ListOrdersResponse{Items: pbItems, Total: total}, nil
}

// AdminGetOrder 管理员查询任意订单详情 RPC（不校验归属）。
func (h *OrderHandler) AdminGetOrder(ctx context.Context, req *orderv1.AdminGetOrderRequest) (*orderv1.GetOrderResponse, error) {
	if req.GetOrderId() <= 0 {
		return nil, pkgerrors.ToGRPCStatus(pkgerrors.New(pkgerrors.CodeInvalidArg, "order_id 必须为正数")).Err()
	}
	order, err := h.orderUC.AdminGetOrderByID(ctx, req.GetOrderId())
	if err != nil {
		return nil, pkgerrors.ToGRPCStatus(err).Err()
	}
	return &orderv1.GetOrderResponse{Order: orderToProto(order)}, nil
...
// AdminCancelOrder 管理员关闭任意未支付订单 RPC（不校验归属）。
func (h *OrderHandler) AdminCancelOrder(ctx context.Context, req *orderv1.AdminCancelOrderRequest) (*orderv1.CancelOrderResponse, error) {
	if req.GetOrderId() <= 0 {
		return nil, pkgerrors.ToGRPCStatus(pkgerrors.New(pkgerrors.CodeInvalidArg, "order_id 必须为正数")).Err()
	}
	order, err := h.orderUC.AdminCancelOrder(ctx, req.GetOrderId())
	if err != nil {
		return nil, pkgerrors.ToGRPCStatus(err).Err()
	}
	return &orderv1.CancelOrderResponse{Order: orderToProto(order)}, nil
