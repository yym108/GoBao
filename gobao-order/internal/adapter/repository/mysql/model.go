...
type OrderModel struct {
	...
	ExpireAt      time.Time        `gorm:"column:expire_at;not null;index"`                            // 支付超时时间
	ClosedAt      *time.Time       `gorm:"column:closed_at"`                                           // 订单关闭时间
	CloseReason   string           `gorm:"column:close_reason;type:varchar(32);not null;default:''"`   // 订单关闭原因
...
type OrderItemModel struct {
	...
	SKUID         int64     `gorm:"column:sku_id;not null;index"`                           // 兼容旧表结构保留，占位写入 0
	SKUCode       string    `gorm:"column:sku_code;type:varchar(100);not null;default:''"`  // 兼容旧表结构保留，占位写空串
	SKUTitle      string    `gorm:"column:sku_title;type:varchar(200);not null;default:''"` // 兼容旧表结构保留，占位写空串
	...
	OptionSummary string    `gorm:"column:option_summary;type:varchar(255)"`                // 商品版本规格摘要
