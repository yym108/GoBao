...
func TestOrderRepo_CreateAndFindByID(t *testing.T) {
	...
	if !got.ExpireAt.Equal(order.ExpireAt) {
		t.Fatalf("expire_at want %v, got %v", order.ExpireAt, got.ExpireAt)
	}
	...
	if got.Items[0].ProductID != order.Items[0].ProductID {
		t.Fatalf("product id want %d, got %d", order.Items[0].ProductID, got.Items[0].ProductID)
	}
	if got.Items[0].Name != order.Items[0].Name || got.Items[0].OptionSummary != order.Items[0].OptionSummary {
		t.Fatalf("product snapshot not persisted, got name=%q option=%q", got.Items[0].Name, got.Items[0].OptionSummary)
	}
...
func TestOrderRepo_UpdateStatus(t *testing.T) {
	...
	order := &domain.Order{
		...
		UpdatedAt:     now,
		Items: []domain.OrderItem{
			{ProductID: 1001, Price: 999900, Quantity: 1, Amount: 999900},
		},
	}
	if err := repo.Create(ctx, order); err != nil {
		t.Fatalf("create: %v", err)
	...
	updated, err := repo.UpdateStatus(
		...
		now.Add(time.Minute),
		domain.OrderCloseReasonUserCancelled,
		&closedAt,
	)
	if err != nil {
	...
	if got.CloseReason != domain.OrderCloseReasonUserCancelled {
		t.Fatalf("unexpected close reason: %s", got.CloseReason)
	}
	if got.ClosedAt == nil || !got.ClosedAt.Equal(closedAt) {
		t.Fatalf("unexpected closed_at: %+v", got.ClosedAt)
	}

	updated, err = repo.UpdateStatus(ctx, order.ID, domain.OrderStatusCreated, domain.OrderStatusCancelled, now.Add(2*time.Minute), domain.OrderCloseReasonTimeout, nil)
	if err != nil {
...
func TestOrderRepo_UpdateStatus_ToPaidAndFailed(t *testing.T) {
	...
	updated, err := repo.UpdateStatus(ctx, order.ID, domain.OrderStatusCreated, domain.OrderStatusPaid, now.Add(time.Minute), "", nil)
	if err != nil {
	...
	other, err := repo.UpdateStatus(ctx, order.ID, domain.OrderStatusPaid, domain.OrderStatusPaymentFailed, now.Add(2*time.Minute), "", nil)
	if err != nil {
