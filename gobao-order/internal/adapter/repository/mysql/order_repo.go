...
// ListAll 管理员分页查询全部订单，可选按状态与用户过滤，按 id 倒序返回最新订单。
// status 为空字符串时不过滤状态；userID 为 0 时不过滤用户。
func (r *OrderRepo) ListAll(ctx context.Context, status string, userID int64, page, pageSize int) ([]*domain.Order, int64, error) {
	var total int64
...
// 当 closeReason 或 closedAt 有值时，会一并写入关闭元数据，供超时关闭和用户取消复用。
func (r *OrderRepo) UpdateStatus(ctx context.Context, id int64, fromStatus, toStatus string, updatedAt time.Time, closeReason string, closedAt *time.Time) (bool, error) {
