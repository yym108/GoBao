...
export async function fetchPayment(paymentId: number): Promise<Payment> {
  const raw = await apiRequest<any>(`/api/v1/payments/${paymentId}`, undefined, { adminAuth: true });
...
export async function fetchPaymentByOrder(orderId: number): Promise<Payment | null> {
  try {
    const raw = await apiRequest<any>(`/api/v1/payments/by-order/${orderId}`, undefined, { adminAuth: true });
